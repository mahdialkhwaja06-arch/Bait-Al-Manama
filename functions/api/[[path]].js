/**
 * Bait Al-Manama — Cloudflare Pages Function
 * Handles all /api/* routes
 * KV binding name: KV  (configure in Cloudflare dashboard)
 */

/* ── Base64 → ArrayBuffer (Workers don't have Node's Buffer) ── */
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (method==='POST' && path_raw==='/section-status') {
      await writeKV('section-status:data', body);
      await bumpVersion();
    ...
    if (method==='DELETE' && cSecMatch) {
      ...
      delete ss[secKey];
      await writeKV('section-status:data', ss);
      await bumpVersion();
      return respond({ ok:true });
    }
    ...
    if (method==='POST' && path_raw==='/media/chunk') {
      ...
      if (!uploadId || chunkIndex === undefined || !data)
        return respond({ error:'uploadId, chunkIndex, data required' }, 400);
      await kv.put('media:chunk:' + uploadId + ':' + chunkIndex, base64ToBuffer(data));
      if (chunkIndex === 0) {
        await writeKV('media:chunkmeta:' + uploadId, { mimeType: mimeType||'application/octet-stream', totalChunks: totalChunks||1 });
      }
    ...
    if (method==='POST' && paidMatch) {
      ...
      if (idx !== -1) {
        ...
        orders[idx].payment_method = body.paymentMethod || 'Cash';
      ...
      if (idx !== -1) {
        ...
        try {
          ...
          for (const oi of orderItems) {
            const assign = assignments[oi.name];
            if (!assign) continue;
            const iIdx = invItems.findIndex(i => i.id === assign.inventoryId);
            if (iIdx === -1) continue;
            const servingUnit = assign.servingUnit || invItems[iIdx].unit;
            const deductAmt = convertUnits(assign.usagePerServing * oi.qty, servingUnit, invItems[iIdx].unit);
            invItems[iIdx].quantity = Math.max(0, invItems[iIdx].quantity - deductAmt);
            invChanged = true;
            if (invItems[iIdx].quantity <= 0) {
              for (const mn in assignments) {
                if (assignments[mn].inventoryId === assign.inventoryId) {
                  disabled[mn] = true;
                  disabled['__inv_' + mn] = true;
        ...
        try {
          ...
          for (const oi of orderItems2) {
            if (stock[oi.name] !== undefined && stock[oi.name] !== null) {
              stock[oi.name] = Math.max(0, stock[oi.name] - oi.qty);
              stockChanged = true;
              if (stock[oi.name] <= 0) {
                disabled2[oi.name] = true;
                disabledChanged2 = true;
              }
            }
          }
    ...
    if (method==='POST' && path_raw==='/disabled-items') {
      await writeKV('disabled-items:data', body);
      await bumpVersion();
      return respond({ ok:true });
    }
    ...
    if (method==='POST' && path_raw==='/custom-items') {
      if (!body.name || body.price === undefined || !body.section)
        return respond({ error:'name, price, section required' }, 400);
    ...
    if (method==='PUT' && customEditMatch) {
      ...
      if (idx !== -1) {
        items[idx].name      = body.name;
        items[idx].price     = body.price;
        items[idx].image_url = body.image_url || '';
        items[idx].is_video  = body.is_video || false;
      }
      await writeKV('custom-items:all', items);
      await bumpVersion();
      return respond({ ok:true });
    ...
    if (method==='POST' && path_raw==='/menu-overrides') {
      await writeKV('menu-overrides:data', body);
      await bumpVersion();
      return respond({ ok:true });
    }
    ...
    if (method==='POST' && path_raw==='/auth/change-password') {
      ...
      if (!body.newPassword) return respond({ ok:false, error:'New password cannot be empty' }, 400);
    ...
    if (method==='POST' && path_raw==='/inventory') {
      ...
      const maxId = items.reduce((m,i) => Math.max(m,i.id), 0);
      const ni = { id:maxId+1, name, quantity:Number(quantity), unit, created_at:new Date().toISOString() };
      items.push(ni);
    ...
    if (method==='GET' && path_raw==='/employees') {
      ...
      BASE_EMPLOYEES.forEach(b => {
        if (!emps.find(e => e.name === b.name)) emps.push(b);
      });
      return respond(emps);
    }
